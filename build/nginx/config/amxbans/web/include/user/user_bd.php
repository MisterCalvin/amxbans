...
// Determine the scheme for our Trackback URL
...
if(isset($_POST["edit_ban"]) && isset($_POST["bid"]) && $_SESSION["loggedin"]) {
    ...
    $ban_created = isset($_POST["ban_created"]) ? (int)$_POST["ban_created"] : 0;
...
if(isset($_POST["add_comment"]) && $bid) {
    ...
    error_log("comment_all: " . $config->comment_all);
    error_log("loggedin: " . $_SESSION["loggedin"]);
    error_log("error: " . print_r($error, true));

    // Check if unregistered users are allowed to comment
    if ($config->comment_all == 1 || $_SESSION["loggedin"] == true) {
        if(!$error) {
...
// Ensure player_comid is always set
$ban_details["player_comid"] = GetFriendId($ban_details["player_id"] ?? '');

// Check if the ban reason is custom and set accordingly
$ban_details["custom_reason"] = '';
if (!empty($reasons_with_times) && !in_array($ban_details["ban_reason"], array_column($reasons_with_times, 'reason'))) {
    $ban_details["custom_reason"] = $ban_details["ban_reason"];
    $ban_details["ban_reason"] = '';
    $ban_details["ban_length"] = $ban_details["ban_length"]; // Keep the custom ban length
} else {
    $selected_reason_obj = array_filter($reasons_with_times, function($reason) use ($ban_details) {
        return $reason['reason'] === $ban_details['ban_reason'];
    });
    if (!empty($selected_reason_obj)) {
        $selected_reason_obj = array_values($selected_reason_obj)[0];
        $ban_details["ban_length"] = $selected_reason_obj['static_bantime'];
    }
}
