...
$result = $mysql->query("SELECT timeout FROM ".$config->db_prefix."_serverinfo LIMIT 1");
if ($result) {
    $row = $result->fetch_assoc();
    if ($row && isset($row['timeout'])) {
        $SERVER_TIMEOUT = (int)$row['timeout'];
    }
}

// Define a constant for use in other PHP scripts
define('SERVER_TIMEOUT', $SERVER_TIMEOUT);
