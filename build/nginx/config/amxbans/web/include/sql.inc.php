...
function sql_get_reasons($set_id = 0) {
    global $config, $mysql;
    
    // Fetch all reasons
    $query = $mysql->query("SELECT r.id, r.reason, r.static_bantime, r.pos, 
                            IF(s.setid IS NOT NULL, 1, 0) AS active 
                            FROM `".$config->db_prefix."_reasons` r 
                            LEFT JOIN `".$config->db_prefix."_reasons_to_set` s 
                            ON r.id = s.reasonid AND s.setid = ".$set_id." 
                            ORDER BY r.pos ASC") or die($mysql->error);
...
function sql_get_reasons_list($set_id) {
    global $config, $mysql;
    $query = $mysql->query("SELECT r.id, r.reason FROM `".$config->db_prefix."_reasons` r
                            JOIN `".$config->db_prefix."_reasons_to_set` rs ON r.id = rs.reasonid
                            WHERE rs.active = 1 AND rs.setid = ".(int)$set_id."
                            ORDER BY r.pos ASC") or die ($mysql->error);
    $reasons = array();
    while ($result = $query->fetch_object()) {
        $reasons[$result->id] = html_safe($result->reason);
    }
    return $reasons;
}


function reindex_reasons() {
    global $config, $mysql;
    $query = $mysql->query("SELECT `id` FROM `".$config->db_prefix."_reasons` ORDER BY `pos` ASC") or die ($mysql->error);
    $pos = 0;
    while ($result = $query->fetch_object()) {
        $mysql->query("UPDATE `".$config->db_prefix."_reasons` SET `pos` = $pos WHERE `id` = ".$result->id) or die ($mysql->error);
        $pos++;
    }
}
