...
*/
    ...
    if(isset($_POST["optimize"]) && $_SESSION["loggedin"]) {
        $tables = ""; // Initialize the variable
        ...
        if ($tables) {
            $query = $mysql->query("OPTIMIZE TABLE ".$tables);
            $user_msg="_DBOPTIMIZED";
        } else {
            $user_msg="_DBOPTIMIZEFAILED";
        }
    }
    ...
    if (isset($_GET['download_plugins']) && $_SESSION["loggedin"]) {
        $source = $config->path_root . '/plugins';
        $zipFile = $config->path_root . '/include/files/plugins.zip';
    
        if (!file_exists($source)) {
            error_log("Source directory does not exist: $source");
            $user_msg = "_SOURCEDOESNOTEXIST";
        } else if (!is_writable($config->path_root . '/include/files/')) {
            error_log("Destination directory is not writable: " . $config->path_root . '/include/files/');
            $user_msg = "_DESTINATIONNOTWRITABLE";
        } else if (create_zip($source, $zipFile)) {
