...
*/
    ...
    if((isset($_POST["rconcommand"]) || isset($_POST["rconuserstart_".$sid])) && $sid) {
        ...
        if(isset($_POST["command"])) {
            ...
            switch($pre) {
                ...
                case 'restart': $command="restart";$max_response_pages=3;break; // This is a forbidden RCON command. Why is it in our list?; ShadesBot
                case 'status': $command="status";break;
                case 'plugins': $command="amx_plugins";$max_response_pages=3;break;
                case 'modules': $command="amx_modules";$max_response_pages=10;break;
    ...
    if(isset($_POST["save"]) && $_SESSION["loggedin"]) {
        $timeout = isset($_POST["timeout"]) ? (int)$_POST["timeout"] : $SERVER_TIMEOUT;
    ...
    $motd_url="https://".str_replace(basename($motd_url),"motd.php?sid=%s&adm=%d&lang=%s",$motd_url);
    ...
    $smarty->assign("timeout", $SERVER_TIMEOUT);
