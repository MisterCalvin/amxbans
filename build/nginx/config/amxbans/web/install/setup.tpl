{assign var="select_lang" value=$current_lang|default:null}
{assign var="default_lang" value=$default_lang|default:"english"}
...
<div class="center-wrapper">
    ...
    <div id="site-title" style="text-align: left;">
        <form method="POST" style="display:inline;">
            ...
            <select name="newlang" onchange="this.form.submit()">
                {foreach from=$lang item=language}
                    <option value="{$language|escape}" {if $select_lang == $language}selected{/if}>{$language|escape}</option>
                {/foreach}
            </select>
        </form>
    </div>
    ...
    <div align="right">
        <input type="hidden" name="site" value="{$sitenr}" />
            ...
            {if isset($checkvalue)}<input type="submit" class="button" name="check{$sitenr}" value="{$checkvalue|lang}" />{/if}
