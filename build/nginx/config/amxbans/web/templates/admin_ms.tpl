...
{if $smarty.session.websettings_view == "yes"}
    ...
    <table width="95%" align="center"><tr><td>
		...
		<table border="1" width="50%" align="center">
			<tr class="htable" colspan="20">
				<td colspan="2"><b>{"_EMAILSETTINGS"|lang}</b></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILHOST"|lang}</td>
				<td><input type="text" size="30" name="email_host" value="{$email_settings.email_host}" /></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILHOSTPORT"|lang}</td>
				<td><input type="text" size="5" name="email_host_port" value="{$email_settings.email_host_port}" /></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILSECURITY"|lang}</td>
				<td>
					<select name="email_security">
						<option value="STARTTLS" {if $email_settings.email_security == "STARTTLS"}selected{/if}>{"_EMAILSTARTTLS"|lang}</option>
						<option value="SMTPS" {if $email_settings.email_security == "SMTPS"}selected{/if}>{"_EMAILSMTPS"|lang}</option>
						<option value="None" {if $email_settings.email_security == "None"}selected{/if}>{"_EMAILINSECURE"|lang}</option>
					</select>
				</td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILUSERNAME"|lang}</td>
				<td><input type="text" size="30" name="email_username" value="{$email_settings.email_username}" /></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILPASSWORD"|lang}</td>
				<td><input type="password" size="30" name="email_password" value="{$email_settings.email_password}" /></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILFROM"|lang}</td>
				<td><input type="text" size="30" name="email_from" value="{$email_settings.email_from}" /></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_EMAILFROMNAME"|lang}</td>
				<td><input type="text" size="30" name="email_from_name" value="{$email_settings.email_from_name}" /></td>
			</tr>
			<tr class="settings_line">
				<td width="50%">{"_TESTEMAILRECIPIENT"|lang}</td>
				<td><input type="text" size="30" name="test_email_recipient" /></td>
			</tr>
		</table>
		<div align="right">
			<input type="submit" class="button" name="test_email" value="{"_TESTEMAIL"|lang}" {if $smarty.session.websettings_edit !== "yes"}disabled{/if} />
		</div>
		<br />
		<div align="right">
			<input type="submit" class="button" name="save" value="{"_SAVE"|lang}" onclick="return confirm('{"_SAVESETTINGS"|lang}');" {if $smarty.session.websettings_edit !== "yes"}disabled{/if} />
		</div>
        </form>
    </table>
{else}
