...
<td id="main" valign="top">
    {if $smarty.session.servers_edit == "yes"}
        ...
        <table width="60%" align="center"><tr><td>
                            ...
                            <td colspan="3"><div style="display: none">
                                <table class="table_details" width="95%">
                                    <form method="POST">
                                        ...
                                        <tr class="settings_line">
                                            <td>{"_DEFAULTBANSET"|lang}:</td>
                                            <td><input type="checkbox" name="default_set" value="1" {if isset($reason.default_set) && $reason.default_set == 1}checked{/if} /></td>
                                            <td></td>
                                        </tr>
                                        <tr class="htable">
                                        ...
                                        {foreach from=$reasons item=r}
                                            <tr class="settings_line">
                                                <td>{$r.reason}</td>
                                                <td>{$r.static_bantime}</td>
                                                <td><input type="checkbox" name="aktiv[]" value="{$r.id}" {if in_array($r.id, explode(',', $reason.reasonids))}checked{/if} /></td>
                                            </tr>
                                        {/foreach}
                                    </form>
                                </table></div>
            ...
            <table border="1" width="100%">
                <tr class="htabletop">
                    <td colspan="4"><b>{"_REASONS"|lang}</b></td>
                </tr>
                ...
                {foreach from=$reasons item=reason name=reasons}
                    <form method="POST">
                    <input type="hidden" name="rid" value="{$reason.id}" />
                    <input type="hidden" name="pos" value="{$reason.pos}" />
                    <tr class="settings_line">
                        <td class="settings_line">
                            <nobr>
                                {if $smarty.foreach.reasons.index > 0}
                                <input type="image" src="images/pfeilo.gif" name="pos_up" border="0" width="9px" height="8px" />
                                {/if}
                                {if $smarty.foreach.reasons.index < $smarty.foreach.reasons.total - 1}
                                <input type="image" src="images/pfeilu.gif" name="pos_dn" border="0" width="9px" height="8px" />
                                {/if}
                            </nobr>
                        </td>
                        <td align="center"><input type="text" name="reason" value="{$reason.reason}" /></td>
                        <td align="center"><input type="text" name="static_bantime" value="{$reason.static_bantime}" /></td>
                        <td align="center">
